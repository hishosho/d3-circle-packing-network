...
export default class CirclePackingNetGraph {
  ...
  render () {
    ...
    const texts = regions.selectAll('text')
      .data((d, i) => {
        return this.regionsPackData[i]
      })
      .enter().append('text')
      ...
      .style('display', (d) => {
          return d.parent === null ? "inline" : "none";
      })
      .text((d) => {
          return d.data.name
      })
  ...
  zoomed () {
    ...
    this.svg.selectAll("text")
        ...
        if (d.parent === null && transform.k < 2 || (d.parent !== null && transform.k >= 2)) {
          return 1
        } else {
          return 0
        }
  ...
  setPath (d) {
      const sourceX = parseFloat(d3.select(`#${d.source}`).attr('cx')),
            ...
            sourceR = parseFloat(d3.select(`#${d.source}`).attr('r')),
  ...
  getTranslate (translateText) {
    const start = translateText.indexOf('(')
    const comma = translateText.indexOf(',')
    const end = translateText.indexOf(')')
    return [ parseFloat(translateText.slice(start + 1, comma)), parseFloat(translateText.slice(comma + 1, end))]
  }
  refreshLink (nodeId) {
